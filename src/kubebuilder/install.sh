...
get_version() {
    ...
    if [ "$VERSION" = "latest" ]; then
        log "INFO" "Fetching latest Kubebuilder version from GitHub..."
...
download_binary() {
    ...
    log "INFO" "Downloading Kubebuilder from $URL"

    if ! curl -sLf --fail -o ./kubebuilder "$URL"; then
        log "ERROR" "Failed to download Kubebuilder!"
        exit 1
    fi
...
install_binary() {
    ...
    log "INFO" "Kubebuilder installed successfully to /usr/local/bin/kubebuilder"
}

log "INFO" "Activating feature 'kubebuilder'"
