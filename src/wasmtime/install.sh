...
get_version() {
    ...
    if [ "$VERSION" = "latest" ]; then
        log "INFO" "Fetching latest Wasmtime version from GitHub..."
...
download_binary() {
    ...
    log "INFO" "Downloading Wasmtime from $URL"

    if ! curl -sLf --fail -o ./wasmtime.tar.xz "$URL"; then
        log "ERROR" "Failed to download Wasmtime!"
        exit 1
    fi
...
install_binary() {
    log "INFO" "Installing Wasmtime..."
    xz -d ./wasmtime.tar.xz
    tar -xof ./wasmtime.tar
    install -m 0755 ./wasmtime-v${VERSION}-${ARCH}-linux/wasmtime /usr/local/bin/wasmtime
    log "INFO" "Wasmtime installed successfully to /usr/local/bin/wasmtime"
}

log "INFO" "Activating feature 'wasmtime'"
