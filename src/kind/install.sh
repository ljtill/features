...
get_version() {
    ...
    if [ "$VERSION" = "latest" ]; then
        log "INFO" "Fetching latest Kind version from GitHub..."
...
download_binary() {
    ...
    if ! curl -sLf --fail -o ./kind "$URL"; then
        log "ERROR" "Failed to download Kind!"
        exit 1
    fi
...
install_binary() {
    ...
    log "INFO" "Kind installed successfully to /usr/local/bin/kind"
}

log "INFO" "Activating feature 'kind'"
