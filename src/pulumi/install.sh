...
get_version() {
    ...
    if [ "$VERSION" = "latest" ]; then
        log "INFO" "Fetching latest Pulumi version from GitHub..."
...
download_binary() {
    ...
    log "INFO" "Downloading Pulumi from $URL"

    if ! curl -sLf --fail -o ./pulumi.tar.gz "$URL"; then
        log "ERROR" "Failed to download Pulumi!"
        exit 1
    fi
...
install_binary() {
    ...
    log "INFO" "Pulumi installed successfully to /usr/local/bin/"
}

log "INFO" "Activating feature 'pulumi'"
