...
get_version() {
    ...
    if [ "$VERSION" = "latest" ]; then
        log "INFO" "Fetching latest Pkl version from GitHub..."
...
download_binary() {
    ...
    URL="https://github.com/apple/pkl/releases/download/${VERSION}/pkl-linux-${ARCH}"
    log "INFO" "Downloading Pkl binary from $URL"

    if ! curl -sLf --fail -o ./pkl "$URL"; then
        log "ERROR" "Failed to download Pkl binary!"
        exit 1
    fi
...
install_binary() {
    log "INFO" "Installing Pkl..."
    install -m 0755 ./pkl /usr/local/bin/pkl
    log "INFO" "Pkl installed successfully to /usr/local/bin/pkl"
}

log "INFO" "Activating feature 'pkl'"
