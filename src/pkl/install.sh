...
get_version() {
    ...
    if [ "$VERSION" = "latest" ]; then
        echo "Fetching latest Pkl version from GitHub..."
...
download_binary() {
    ...
    echo "Downloading Pkl binary from $URL"

    if ! curl -sLf --fail -o ./pkl "$URL"; then
        echo "Failed to download Pkl binary!"
        exit 1
    fi
...
install_binary() {
    ...
    echo "Pkl installed successfully to /usr/local/bin/pkl"
}

echo "Activating feature 'pkl'"
