...
get_version() {
    ...
    if [ "$VERSION" = "latest" ]; then
        log "INFO" "Fetching latest Ruff version from GitHub..."
...
download_binary() {
    ...
    log "INFO" "Downloading Ruff binary from $URL"

    if ! curl -sLf --fail -o ./ruff.tar.gz "$URL"; then
        log "ERROR" "Failed to download ruff binary!"
        exit 1
    fi
...
install_binary() {
    ...
    log "INFO" "Ruff installed successfully to /usr/local/bin/ruff"
}

log "INFO" "Activating feature 'ruff'"
