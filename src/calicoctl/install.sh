...
get_version() {
    ...
    if [ "$VERSION" = "latest" ]; then
        log "INFO" "Fetching latest Calicoctl version from GitHub..."
...
download_binary() {
    ...
    log "INFO" "Downloading Calicoctl from $URL"
...
install_binary() {
    log "INFO" "Installing Calicoctl..."
    install -m 0755 ./calicoctl /usr/local/bin/calicoctl
    log "INFO" "Calicoctl installed successfully to /usr/local/bin/calicoctl"
}

log "INFO" "Activating feature 'calicoctl'"
