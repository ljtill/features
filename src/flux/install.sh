...
get_version() {
    ...
    if [ "$VERSION" = "latest" ]; then
        echo "Fetching latest Flux CLI version from GitHub..."
...
download_binary() {
    ...
    echo "Downloading Flux CLI from $URL"

    if ! curl -sLf --fail -o ./flux.tar.gz "$URL"; then
        echo "Failed to download Flux CLI!"
        exit 1
    fi
...
install_binary() {
    ...
    echo "Flux CLI installed successfully to /usr/local/bin/flux"
}
