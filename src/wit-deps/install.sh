...
get_version() {
    ...
    if [ "$VERSION" = "latest" ]; then
        echo "Fetching latest WIT Deps version from GitHub..."
...
download_binary() {
    ...
    echo "Downloading WIT Deps from $URL"

    if ! curl -sLf --fail -o ./wit-deps "$URL"; then
        echo "Failed to download WIT Deps!"
        exit 1
    fi
...
install_binary() {
    ...
    echo "WIT Deps installed successfully to /usr/local/bin/wit-deps"
}
