...
get_version() {
    ...
    if [ "$VERSION" = "latest" ]; then
        echo "Fetching latest Wasm Tools version from GitHub..."
...
download_binary() {
    ...
    echo "Downloading Wasm Tools from $URL"

    if ! curl -sLf --fail -o ./wasm-tools.tar.gz "$URL"; then
        echo "Failed to download Wasm Tools!"
        exit 1
    fi
...
install_binary() {
    ...
    echo "Wasm Tools installed successfully to /usr/local/bin/wasm-tools"
}
